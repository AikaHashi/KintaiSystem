...
const KintaiApp = (function() {
	...
	function loadKintaiTable(dateStr) {
		...
		// 日付表示
		document.getElementById('work-date').textContent = dateStr + ' の勤怠';

		// ユーザー表示
		document.getElementById('userName').textContent = loginUserName;

		// hiddenセット
		document.getElementById('formUserId').value = loginUserId;
		document.getElementById('formUserName').value = loginUserName;
		document.getElementById('formWorkDate').value = dateStr;
		...
		form.reset();
		form.elements['scheduledWorkHours'].value = '';
		form.elements['actualWorkHours'].value = '';
		form.elements['overtimeHours'].value = '';
		form.elements['deductionTime'].value = '';
			...
			.then(data => {
				...
				const setTime = (name, value) => {
					if (value && value.length >= 5) {
						form.elements[name].value = value.substring(0, 5);
					}
				};
